...
function StatusBadge({ status }: Readonly<{ status: string }>) {
  ...
  return (
    <div
      className={`inline-flex items-center justify-center rounded-[0.25rem] border-[1.5px] px-[0.5rem] py-[0.125rem] ${
        isScheduled
...
export default function MeetingDetailPage() {
  ...
  const loadMeeting = useCallback(async () => {
    ...
    try {
      ...
      const found = response.data.find((slot) => slot.slotId === meetingId);
      if (found) {
        setMeeting(found);
      } else {
              ...
              <div className="flex w-88 flex-col gap-4 rounded-sm bg-white p-5 shadow-[0_0_0.5rem_rgba(0,0,0,0.1)]">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <DetailField
                    ...
                    value="Virtual (Zoom)"
                  />
                  <DetailField label="Location" value="Online" />
                  <DetailField
                    label="Instructions"
